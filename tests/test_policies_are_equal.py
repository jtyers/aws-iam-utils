...
def reorder_policy(p):
    """Returns a copy of p which has had its Statements and nested Actions re-ordered
    randomly."""
    result = copy.deepcopy(p)
    random.shuffle(result["Statement"])

    for st in result["Statement"]:
        if type(st["Action"]) is list:
            random.shuffle(st["Action"])
...
def test_policies_equal_when_inputs_equal_but_reordered(
    policy_1_reordered, policy_1_wildcards
):
    assert aws_iam_utils.checks.policies_are_equal(
        policy_1_reordered, lowercase_policy(policy_1_wildcards)
    )
    ...
    p1 = create_policy(
        {
            ...
            "Action": "s3:PutObject",
            "Resource": "*",
            "Condition": {"StringNotEqual": {"foo": "bar"}},
        }
    )
    ...
    p2 = create_policy(
        {
            ...
            "Action": "s3:putobject",
            "Resource": "*",
            "Condition": {"StringNotEqual": {"foo": "baz"}},
        }
    )
