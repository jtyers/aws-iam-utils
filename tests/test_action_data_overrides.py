...
def test_policy_sentry_actions_out_of_date():
    ...
    assert get_action_data('lambda', 'GetFunctionUrlConfig') is False
...
def test_get_action_data_with_overrides_calls_get_action_data():
    action = 's3:listbucket'
    service_name, action_name = action.split(':')

    assert action not in ACTION_DATA_OVERRIDES
